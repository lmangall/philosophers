...
typedef struct s_philo
{
	...
	int eating;          
		/**< Flag indicating if the philosopher is currently eating. */
	int fork_l;           /**< Philosopher's left fork. */
	int fork_r;           /**< Philosopher's right fork. */
	uint64_t last_eat;   
		/**< Time in milliseconds when the philosopher last ate. */
	uint64_t tto_die;    
		/**< Time in milliseconds when the philosopher last ate. */
...
typedef struct s_data
{
	...
	uint64_t tto_die;       
		/**< Time in milliseconds before a philosopher dies if they haven't eaten. */
	uint64_t tto_eat;       
		/**< Time in milliseconds required for a philosopher to eat. */
	uint64_t tto_sleep;     
		/**< Time in milliseconds required for a philosopher to sleep. */
	t_philo *philos;         /**< Array of philosophers. */
	pthread_t *death_thread; /**< Philosopher's thread. */
	pthread_t		*meals;
	pthread_mutex_t *forks; /**< Array of mutex locks for the forks. */
	pthread_mutex_t lock;  
		/**< Mutex lock for controlling access to shared data. */
